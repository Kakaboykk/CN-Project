...
def evaluate_performance(models, X, y, edge_index, device='cpu'):
    ...
    for name, model in models.items():
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(
            y.view(-1), pred_ts.view(-1), average='macro', zero_division=0.0
        )
        ...
        # Handle models that may not have been trained yet
        loss_train = model.stat['loss_train'][-1] if hasattr(model, 'stat') and 'loss_train' in model.stat else 0.0
        loss_val = model.stat['loss_val'][-1] if hasattr(model, 'stat') and 'loss_val' in model.stat else 0.0
        acc_train = model.stat['acc_train'][-1] if hasattr(model, 'stat') and 'acc_train' in model.stat else 0.0
        acc_val = model.stat['acc_val'][-1] if hasattr(model, 'stat') and 'acc_val' in model.stat else 0.0
        ...
        d = {'model': name, 'TN': TN, 'FP': FP, 'FN': FN, 'TP': TP,
                ...
                'loss_train': '{:.4f}'.format(loss_train), 'loss_val': '{:.4f}'.format(loss_val),
                'acc_train': '{:.4f}'.format(acc_train), 'acc_val': '{:.4f}'.format(acc_val),
